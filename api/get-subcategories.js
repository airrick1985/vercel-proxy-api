...
export default async function handler(req, res) {
  ...
  if (!category || token !== 'anxi111003') {
    return res.status(400).json({ status: 'error', message: '缺少 category 或 Token 驗證失敗' });
  }
  ...
  try {
    const gasRes = await fetch(GAS_URL, {
      ...
      body: JSON.stringify({ action: 'get_subcategories', category })
    });
